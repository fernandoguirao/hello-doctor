package com.hellodoctor.screens
{
	...
	public class ConsultaOculista extends MovieClip
	{
		...
		private var tipoModal:Boolean;

		public function ConsultaOculista()
		{
			// constructor code
			super();
			//trace("Recepcion");
		...
		public function init( screenData:Object ):void
		{
			...
			llamada.mc.jugar.visible = false;
			llamada.mc.explicar.visible = false;
			/*
			controller:this
			isFirstTime:_nuevo
			sintoma:_sintoma
			...
			*/
			...
			{
				_controller._primeraOculista = false;
		...
		private function reproduce():void
		{
			...
			else if (charlas[countCharlas][0] == "pausa")
			{
				if (!tipoModal)
				{
					trace("espera");
					trace("modal1");
					llamada.desc.visible = true;
					llamada.mc.visible = false;
					//tipoModal = true;
				...
				else
				{
					...
					if (!hayLlamada)
					{
						llamada.desc.visible = false;
						llamada.mc.visible = true;
						llamada.visible = true;
		...
		private function onModal( e:MouseEvent ):void
		{
			...
			else
			{
				...
				switch (btn)
				{
					case (llamada.mc.jugar) :
						_controller.onTweenScreen(Constants.REQUEST_JUEGO_OCULISTA);
						break;
					
					
					case (llamada.mc.explicar) :
						init(_objAll);
						break;
			...
			if (!_controller._pausaTodo || esperaModal)
			{
				...
				if (channel.position > stopSound)
				{
					...
					else
					{
						...
						{
							llamada.mc.jugar.visible = true;
							llamada.mc.explicar.visible = true;
							llamada.mc.jugar.addEventListener(MouseEvent.CLICK, onModal);
							llamada.mc.explicar.addEventListener(MouseEvent.CLICK, onModal);
							//_controller.onTweenScreen(Constants.REQUEST_JUEGO_OCULISTA);
						}
						else
						{
